	...
	FileUpload.prototype = {
		...
		_Init: function(data){
			...
			for(var i = 0, len = data.length; i < len; i++){
				html += this._GenerateHtmlUnit('loaded', data[i]);
			}
		...
		_UploadDone: function(ajaxId, data){
			...
			if(self.limit && uploaded == self.limit) {
				...
				self.$table.find('[data-ajaxId="' + ajaxId + '"]').replaceWith(this._GenerateHtmlUnit.call(self, 'loaded', url));
			}			
		},
...
//			}
		...
		_GenerateHtmlUnit: function(status, src, msg){
			//如果上传其他文件，在这里修改模板就好了
			...
			var loadedTpl = '<tr data-src="{src}" class="file-uploaded">\
							<td class="radio-paly"><img src="{src}"></td>\
							<td>\
								<p>已上传</p>\
							</td>\
							<td>\
								<input type="hidden" name="{name}" value="{src}" class="file-uploaded-input">\
                <label class="collapse file-uploaded-input-data">{src-data}</label>\
			...
			var html = '';
			if(status == 'loaded') {
				html = loadedTpl.replace(/{src}/ig, src.b).replace(/{src-data}/ig, JSON.stringify(src)).replace(/{name}/ig, this.inputName);
			}
	...
	if(window.FileUpload === undefined) {
