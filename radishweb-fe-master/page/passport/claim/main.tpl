...
<!--             <a href="/radishweb/question/info?id={$item.id}">-->
              ...
              <div class="info pull-right p-w91 p-b15" data-id="{$item.id}" data-open="{$item.open}">
                ...
                <P class="m-b10"><label>联系人：</label>{$item.name} </P>
                <P class="m-b10"><label class="">联系电话：</label>{$item.phone}</P>
                </a>  
                  {if $session.type == '2'}
                  <!-- <a class="btn-a" href="/radishweb/question/barcode?id={$item.id}&Release=y">打印条形码</a> -->
                {/if}
                <span>
                <a class="btn-a information" href="/radishweb/question/claiminfo?id={$item.claim.id}">认领信息</a>
                <a class="btn-a btn-delete" style="border: 1px solid red;color: red;" data-toggle="modal" data-target="#delete-confirm">删除信息</a>
