      ...
      <div class="col-list m-t20">
        <ul>
          ...
          <li class="m-b25">
            ...
            <a href="/radishweb/question/info?id={$item.question.id}" data-id2="{$session.id}">
              {if $session.avatar}<img src="{$session.avatar.s}" class="pull-left imgWrap" width="60" height="60">{else}<img src="/images/avatar2.png" width="60" height="60" class="pull-left imgWrap" >{/if}
              <div class="info pull-right p-w91">
                ...
                {/if}
                <p class="content m-b10">{$item.question.content|truncate:120:"...":true}</p>
                <P class="m-b25"><label>联系人：</label>{$item.question.name} <label class="m-l20">联系电话：</label>{$item.question.phone}<span class="cancelled" data-id="{$item.id}">删除</span></P>
              </div>
            </a>
          </li>
          {/foreach}
