				...
				<div class="Survey">
					<div class="Surveydiv">
						...
						<p style="text-align: left;">
							{if $info.left_num}
							<span style="color:red;margin-right:65px;font-size:24px"><i class="left">{$info.left_num }</i>条</span>
							{else}
							<span style="color:red;margin-right:65px;font-size:24px"><i class="left">0</i>条</span>
							{/if}
							<button class="btn btn-warning Recharge" style="width:192px;line-height:27px;">立即充值</button>
						</p>
					</div>
					<table class="table">
						<thead>
						...
						{if $fee}
						 {foreach from=$fee key=i item=item}
							<tr class="Template">
								<td></td>
								<td class="time">{$item.create_time}</td>
								<td class="count">{$item.count}</td>
								<td class="used">{$item.used_num}</td>
								<td class="left">{$item.left_num}</td>
								<td class="toname">{$item.to_name}</td>
							</tr>
						{/foreach}
						{else}
						<tr class="Template">
