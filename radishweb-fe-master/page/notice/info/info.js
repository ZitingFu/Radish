(function (window, $, undefined) {
        ...
        $(document).on('click', 'a.com', function () {
            ...
            .on('click', 'a.pra', function () {
                ...
                $.post('/radishweb/notice/editlike',{id:id}, function(data) {
                    //console.log(data);
                });
            });
        var regs=/\S/;
...
//pre
        ...
        //com
        ybb.ajaxSubmit({
            url: '/radishweb/comments/noticecreate2',
            form: '#comments form',
    ...
    //一级场所
        var li_index6= $('div.sidebar ul li:gt(6)');
        li_index6.hide();
        $('div.sidebar').find('button').on('click',function(){
            if(li_index6.is(":hidden")){
                li_index6.show();
                $(this).html('显示部分');
            }else {
                li_index6.hide();
...
//打赏部分
    ...
    $('#withdraw').on('click',function(){
        ...
        $.post('/radishweb/reword/create',{price:(dataMoney*100),q_id:topicId},function(data){
            if(data.flag ==1){
                ...
                if(data.msg == '请先设置提现密码'){
                    setTimeout(function () {
                        location.href='/radishweb/passport/createwithdrawpass?id='+topicId+'&type=2' ;
                    }, 1000);
                }
                return
            }
    ...
    $('#topicPass').on('click',function(){
        ...
        //订单ID。 密码
        $.post('/radishweb/reword/paynotice',{id:$('#orderId').val(),pass:passw,pay:4},function(data){
            if(data.flag=='1'){
                if(data.msg =="提现密码不对"){
