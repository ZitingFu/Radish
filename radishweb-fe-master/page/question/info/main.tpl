    ...
    <div class="content ">
      <div class="details clearFix m-b50" data-id2="{$info.user_id}">
        ...
        <div class="info pull-right p-w90">
          <p class="clearFix"> <label class="pull-left">{$info.user_name}</label> <label class="grey m-l20 pull-left m-t2">爱心值 · {$info.love}  发布数 · {$info.issue_num}   找到数 · {$info.find_num} </label> <label class="pull-right">{$info.time}</label> </p>
          <p class="m-t10 m-b10">{$info.genre_name}</p>
          ...
          <p class="content m-b20">{$info.content}</p>
          ...
          <p  class="m-b30" style="display: block"><a href="/radishweb/question/map?id={$info.id}" target='_blank'><img src="/images/map.png" alt="" width="10px" height="15px" style="margin-top: -4px"> &nbsp;{$info.site_two_name}</a></p>
          {/if}
          ...
          {if $fav==99}
          <button type="button" class="btn btn-outline btn-warning   h-28 collection" style="margin-left:0;padding:5px 35px;line-height:0px;height:28px">收藏</button>
          {else}
          <button type="button" class="btn  btn-warning   h-28 collection" style="margin-left:0">已收藏</button>
          {/if}
          {/if}
        </div>
        <div id="alImg">
